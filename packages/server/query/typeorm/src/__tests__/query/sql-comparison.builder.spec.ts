...
jest.mock('../../lib/common/randomString', () => ({ randomString: jest.fn() }));
...
describe('SQLComparisonBuilder', (): void => {
  ...
  it('should throw an error for an invalid comparison type', () => {
    ...
    expect(() =>
      // @ts-ignore
      createSQLComparisonBuilder().build('stringType', 'bad', 'foo', 'TestEntity')
    ).toThrow('unknown operator "bad"');
  });
  ...
  describe('like comparisons', () => {
    it('should build gte sql fragment', (): void => {
      expect(
        createSQLComparisonBuilder().build('stringType', 'like', '%hello%', 'TestEntity')
      ).toEqual({
  ...
  describe('notLike comparisons', () => {
    it('should build gte sql fragment', (): void => {
      expect(
        createSQLComparisonBuilder().build('stringType', 'notLike', '%hello%', 'TestEntity')
      ).toEqual({
  ...
  describe('iLike comparisons', () => {
    it('should build gte sql fragment', (): void => {
      expect(
        createSQLComparisonBuilder().build('stringType', 'iLike', '%hello%', 'TestEntity')
      ).toEqual({
  ...
  describe('notILike comparisons', () => {
    it('should build gte sql fragment', (): void => {
      expect(
        createSQLComparisonBuilder().build('stringType', 'notILike', '%hello%', 'TestEntity')
      ).toEqual({
  ...
  describe('is comparisons', () => {
    ...
    it('should throw an error for values other than null true or false', () => {
      expect(() =>
        // @ts-ignore
        createSQLComparisonBuilder().build('boolType', 'is', 'foo', 'TestEntity')
      ).toThrow('unexpected is operator param "foo"');
    });
  });
  ...
  describe('in comparisons', () => {
    ...
    it('should throw an error for empty array', (): void => {
      const arr: number[] = [];
      expect(() =>
        createSQLComparisonBuilder().build('numberType', 'in', arr, 'TestEntity')
      ).toThrow('Invalid in value expected a non-empty array got []');
    });
  ...
  describe('notIn comparisons', () => {
    ...
    it('should throw an error for empty array', (): void => {
      const arr: number[] = [];
      expect(() =>
        createSQLComparisonBuilder().build('numberType', 'notIn', arr, 'TestEntity')
      ).toThrow('Invalid in value expected a non-empty array got []');
    });

    it('should throw an error for non-array', (): void => {
      expect(() =>
        createSQLComparisonBuilder().build('numberType', 'notIn', 1, 'TestEntity')
      ).toThrow('Invalid in value expected an array got 1');
    });
  });
  ...
  describe('between comparisons', () => {
    it('should build between comparisons', (): void => {
      ...
      expect(
        createSQLComparisonBuilder().build('numberType', 'between', between, 'TestEntity')
      ).toEqual({
  ...
  describe('notBetween comparisons', () => {
    it('should build not between comparisons', (): void => {
      ...
      expect(
        createSQLComparisonBuilder().build('numberType', 'notBetween', between, 'TestEntity')
      ).toEqual({
