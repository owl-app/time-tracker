...
import { Filter, SortDirection } from '@owl-app/nestjs-query-core';
...
describe('TypeOrmQueryService', (): void => {
  ...
  class TestSoftDeleteEntityService extends TypeOrmQueryService<TestSoftDeleteEntity> {
    constructor(
      @InjectRepository(TestSoftDeleteEntity)
      readonly repo: Repository<TestSoftDeleteEntity>
    ) {
          ...
          const queryResult = await queryService.query({
            filter: {
              oneTestRelation: {
                testRelationPk: {
                  in: [
                    `test-relations-${entity.testEntityPk}-1`,
    ...
    describe('with multiple entities', () => {
      ...
      it('should apply paging', async () => {
        ...
        entities.forEach((e, index) =>
          expect(queryResult.get(e)).toHaveLength(index === 0 ? 2 : 0)
        );
      });
  ...
  describe('#updateOne', () => {
    ...
    it('should reject if the update contains a primary key', async () => {
      const queryService = moduleRef.get(TestEntityService);
      return expect(
        queryService.updateOne(TEST_ENTITIES[0].testEntityPk, {
          testEntityPk: 'bad-id',
        })
