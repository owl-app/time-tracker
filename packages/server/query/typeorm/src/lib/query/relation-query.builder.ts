...
export class RelationQueryBuilder<Entity, Relation> {
  ...
  public select(
    ...
    relationBuilder = hasRelations
      ? this.filterQueryBuilder.applyRelationJoinsRecursive(
          relationBuilder,
          this.filterQueryBuilder.getReferencedRelationsWithAliasRecursive(
  ...
  public batchAggregate(
    ...
    const selects = [
      ...AggregateBuilder.getAggregateSelects(aggregateQuery),
  ...
  public aggregate(
    ...
    let relationBuilder = this.createRelationQueryBuilder(entity);
    relationBuilder = this.filterQueryBuilder.applyAggregate(
      relationBuilder,
      aggregateQuery,
      relationBuilder.alias
    );
  ...
  private getManyToOneOrOneToOneOwnerMeta(
    ...
    return {
      ...
      mapRelations: <RawRelation>(
        ...
        return lodashFilter(rawRelations, rawFilter).reduce<Relation[]>(
          (entityRelations, rawRelation) => {
            const filter = this.getRelationPrimaryKeysPropertyNameAndColumnsName().reduce(
              (columns: Partial<Entity>, column) => ({
                ...columns,

                [column.propertyName]: (rawRelation as unknown as { [key: string]: unknown })[
                  column.columnName
                ],
              }),
              {} as Partial<Entity>
      ...
      batchSelect: (queryBuilder, entities) => {
        ...
        const whereCondition = relation.entityMetadata.primaryColumns
          .map((column) => {
            ...
            whereParams[paramName] = entities.map(
              (entity) => column.getEntityValue(entity) as unknown
            );

            // Also select the columns, so we can use them to map later
            queryBuilder.addSelect(
              `${joinAlias}.${column.propertyPath}`,
      ...
      mapRelations: <RawRelation>(
        ...
        return lodashFilter(rawRelations, rawFilter).reduce((entityRelations, rawRelation) => {
          const filter = this.getRelationPrimaryKeysPropertyNameAndColumnsName().reduce(
            (columnsFilter, column) => ({
              ...columnsFilter,
              [column.propertyName]: (rawRelation as { [key: string]: unknown })[column.columnName],
            }),
            {} as Partial<Entity>
          );
  ...
  private batchMapRelationsManyToMany<RawRelation>(
    ...
    return lodashFilter(rawRelations, rawFilter).reduce((entityRelations, rawRelation) => {
      const filter = this.getRelationPrimaryKeysPropertyNameAndColumnsName().reduce(
        (columnsFilter, column) => ({
          ...columnsFilter,
          [column.propertyName]: (rawRelation as Record<string, unknown>)[column.columnName],
        }),
        {} as Partial<Entity>
      );
