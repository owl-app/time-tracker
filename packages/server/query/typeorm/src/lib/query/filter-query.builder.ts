...
export class FilterQueryBuilder<Entity> {
  ...
  public aggregate(
    ...
    qb = hasFilterRelations
      ? this.applyRelationJoinsRecursive(
          qb,
          this.getReferencedRelationsWithAliasRecursive(this.repo.metadata, query.filter)
        )
      : qb;
  ...
  public applyFilter<Where extends WhereExpressionBuilder>(
    ...
    return this.whereBuilder.build(
      qb,
      filter,
      this.getReferencedRelationsWithAliasRecursive(this.repo.metadata, filter),
      alias
    );
  }
  ...
  public applyRelationJoinsRecursive(
    ...
    return referencedRelations.reduce((rqb, [relationKey, relation]) => {
      ...
      const selectRelation =
        selectRelations && selectRelations.find(({ name }) => name === relationKey);
