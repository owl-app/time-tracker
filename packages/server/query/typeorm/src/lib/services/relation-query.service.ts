...
export abstract class RelationQueryService<Entity> {
  ...
  public async queryRelations<Relation>(
    ...
    return assembler.convertAsyncToDTOs(
      relationQueryBuilder.select(dto, assembler.convertQuery(query)).getMany()
    );
  }
  ...
  public async findRelation<Relation>(
    ...
    let relationEntity = opts?.lookedAhead
      ? (dto[relationName as keyof typeof dto] as unknown)
      : undefined;
  ...
  async setRelations<Relation>(
    ...
    const existingRelations = await relationQueryBuilder
      ...
      .getMany();
    await this.createTypeormRelationQueryBuilder(entity, relationName).addAndRemove(
      relations,
      existingRelations
    );
    return entity;
   ...
   */
  async batchQueryRelations<Relation>(
    RelationClass: Class<Relation>,
    relationName: string,
    entities: Entity[],
    query: Query<Relation>,
   ...
   * @param query - A query to filter, page or sort relations.
   */
  async batchAggregateRelations<Relation>(
    RelationClass: Class<Relation>,
    relationName: string,
    entities: Entity[],
    filter: Filter<Relation>,
   ...
   */
  async batchCountRelations<Relation>(
    RelationClass: Class<Relation>,
    relationName: string,
    entities: Entity[],
    filter: Filter<Relation>
  ...
  private async batchFindRelations<Relation>(
    ...
    if (opts?.lookedAhead) {
      ...
      if (
        (isNullable &&
          dtos.some((entity) => (entity as Entity & { [key: string]: any })[relationName])) ||
        (!isNullable &&
          dtos.some((entity) => (entity as Entity & { [key: string]: any })[relationName]))
      ) {
        const assembler = AssemblerFactory.getAssembler(
        ...
        for (const entity of dtos) {
          results.set(
            entity,
            (entity as Entity & { [key: string]: any })[relationName]
              ? // eslint-disable-next-line no-await-in-loop
                await assembler.convertToDTO(
                  (entity as Entity & { [key: string]: any })[relationName]
                )
              : undefined
          );
  ...
  createTypeormRelationQueryBuilder(
    entity: Entity,
    relationName: string
  ...
  getRelationMeta(relationName: string): RelationMetadata {
  ...
  getRelationEntity(relationName: string): Class<unknown> {
    const relationMeta = this.getRelationMeta(relationName);

    if (typeof relationMeta.type === 'string') {
      const relationMetaData = this.repo.manager.connection.entityMetadatas.find(
  ...
  getRelations<Relation>(
    ...
    const relationQueryBuilder =
      this.getRelationQueryBuilder<Relation>(relationName).filterQueryBuilder;
    return relationQueryBuilder.selectById(ids, { filter }).getMany();
  }

  foundAllRelations<Relation>(relationIds: (string | number)[], relations: Relation[]): boolean {
